...
namespace Pracuj.ath.bielsko.pl.ViewModels
{
    ...
    public class AddJobOffertValidation : AbstractValidator<AddJobOffertViewModel>
    {
        public AddJobOffertValidation()
        {
            ...
            RuleFor(x => x.JobCategory).NotNull().WithMessage("Wybierz branże");
            RuleFor(x => x.ContractType).NotNull().WithMessage("Wybierz typ zatrudnienia");
            ...
            RuleFor(x => x.PostDuration).NotNull().WithMessage("Wprowadz czas trwania ogłoszenia");
        }
    }
}